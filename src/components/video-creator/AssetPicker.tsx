...
import { CheckSquare, Image as ImageIcon, Loader2, AlertTriangle } from 'lucide-react';
...
export function AssetPicker({ open, maxSelect, onClose, onAdd }: AssetPickerProps) {
  ...
  useEffect(() => {
    ...
    authFetch('/api/designs?limit=24')
      .then(async (res) => {
        ...
        setDesigns(items);
        setActiveDesignId(items[0]?.id || null);
      })
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                {designs.map((design) => {
                  ...
                  const thumb = design.thumbnail && isApiFileUrl(design.thumbnail) ? toVideoFilesUrl(design.thumbnail) : '';
                  return (
                    <button
                      key={design.id}
                      type="button"
